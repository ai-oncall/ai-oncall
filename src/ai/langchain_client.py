...
class MessageClassification(BaseModel):
    ...
    type: str = Field(description="Classification type: incident, knowledge_query, support_request, deployment_help")
    severity: str = Field(description="Severity level: low, medium, high, critical")
    urgency: str = Field(description="Urgency level: low, medium, high")
    confidence: float = Field(description="Confidence score between 0.0 and 1.0")
    reasoning: Optional[str] = Field(description="Brief explanation of the classification")


class KnowledgeResponse(BaseModel):
    """Structured output for knowledge base responses."""
    summary: str = Field(description="Main summary of the information")
    key_points: List[str] = Field(description="Key points extracted from the knowledge base")
...
class LangChainAIClient:
    ...
    def _setup_chains(self):
        ...
        self.classification_parser = PydanticOutputParser(pydantic_object=MessageClassification)
        
        classification_template = ChatPromptTemplate.from_messages([
            SystemMessagePromptTemplate.from_template(self._build_classification_system_prompt()),
            HumanMessagePromptTemplate.from_template("Classify this message: {message}\n\n{format_instructions}")
        ])
        
        if self._llm:
            self.classification_chain = (
                {"message": RunnablePassthrough(), "format_instructions": lambda _: self.classification_parser.get_format_instructions()}
...
Provide a helpful response:
    ...
    async def classify_message(self, message: str) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info("Message classified successfully with LangChain",
                       type=result.type,
    ...
    async def generate_knowledge_response(self, user_query: str, search_results: str) -> str:
        ...
        try:
            ...
            logger.info("Knowledge response generated with LangChain",
                       query=user_query,
                       confidence=result.confidence,
            ...
            logger.error("Error generating knowledge response with LangChain", 
                        query=user_query, 
                        error=str(e))
            return self._get_mock_knowledge_response(search_results)
    
    async def generate_response(self, message: str, classification: Dict[str, Any], context: Optional[Dict] = None) -> str:
    ...
    def _load_workflow_config(self):
        ...
        try:
            ...
            if workflow_file.exists():
                ...
                logger.info("Loaded workflow config for LangChain classification", 
