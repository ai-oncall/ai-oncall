...
class LangChainKnowledgeManager:
    ...
    def __init__(
        ...
        collection_name: str = "langchain_knowledge",
    ):
    ...
    def _ensure_initialized(self):
        """Ensure that LangChain components are initialized before use."""
    ...
    def _setup_retrieval_chain(self):
        ...
        try:
            ...
            if not hasattr(self.retrieval_chain, "invoke") and not hasattr(
                self.retrieval_chain, "__call__"
            ):
    ...
    def add_document(
        self, text: str, source: str, filepath: Optional[str] = None
    ) -> str:
    ...
    def _process_file(self, filepath: Path) -> bool:
        ...
        try:
            ...
            self.add_document(
                text=content, source=filepath.name, filepath=str(filepath)
            )

            return True

        except Exception as e:
            logger.error("Error processing file", filepath=str(filepath), error=str(e))
            return False
    ...
    async def search_with_chain(self, query: str, max_results: int = 3) -> str:
        ...
        try:
            ...
            logger.error(error_msg, error=str(e), query=query)
    ...
    async def search_with_relevance(
        ...
        """Search the knowledge base and return results with relevance scores."""
        self._ensure_initialized()
        assert self.vectorstore is not None
        if not LANGCHAIN_AVAILABLE:
        ...
        retriever = self.vectorstore.as_retriever(
            search_kwargs={
                "k": max_results,
                "fetch_k": max_results
                * 2,  # Fetch more candidates for better filtering
        ...
        # Format results with metadata and relevance
        formatted_results = []
        for doc in results:
    ...
    def search(
        self, query: str, max_results: int = 3, similarity_threshold: float = 0.7
    ) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            logger.error("Error during traditional search", error=str(e), query=query)
            raise
