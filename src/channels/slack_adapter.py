...
class SlackAdapter(ChannelAdapter):
    ...
    async def receive_event(self, request: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.exception(
                "Error processing Slack event", event_type=event_type, error=str(e)
            )
            raise
    ...
    async def start_socket_mode(self, message_processor):
        ...
        async def process_socket_event(
            ...
            try:
                if req.type == "events_api":
                    ...
                    if event["type"] == "message" and not event.get("subtype"):
                        ...
                        logger.info(
                            "Received message via Socket Mode",
                            channel=event.get("channel"),
                            user=event.get("user"),
                        ...
                        try:
                            ...
                            if result and result.response:
                                logger.info(
                                    "Sending response via Socket Mode",
                                    channel=context.channel_id,
                                    thread_ts=context.thread_ts,
                            ...
                            logger.exception(
                                "Error processing socket mode message",
                                channel=context.channel_id,
                                error=str(e),
                            )
            except Exception as e:
        ...
        client = SocketModeClient(
            app_token=config.slack_app_token, web_client=self.app.client
        )
        ...
        try:
            ...
            while True:
                try:
                    ...
                    if not is_connected:
                        logger.warning(
                            "Socket Mode disconnected, attempting to reconnect"
                        )
                        await client.connect()
                    await asyncio.sleep(30)  # Check connection every 30 seconds
                except Exception as e:
                    logger.error(
                        "Error in Socket Mode connection maintenance", error=str(e)
                    )
