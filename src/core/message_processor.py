...
class MessageProcessor:
    ...
    async def _classify_message(
        self, ai_client, context: MessageContext, history: list
    ) -> Dict[str, Any]:
    ...
    def _build_classification_prompt(
        self, context: MessageContext, history: list
    ) -> str:
    ...
    async def _generate_response(
        ...
        # Handle knowledge queries first
        if msg_type == "knowledge_query":
            logger.info("Processing knowledge query", query=context.message_text)
        ...
        if workflow_result and workflow_result.get("response"):
            logger.info(
                "Using workflow response", workflow_type=workflow_result.get("type")
            )
            return workflow_result["response"]

        # Generate fallback response using LangChain
        logger.info("Generating fallback response using LangChain")
        return await self.ai_client.generate_response(
            context.message_text, classification
        )

    async def _search_knowledge_base(self, query: str) -> str:
        """Search knowledge base and generate user-friendly response using LangChain."""
        ...
        try:
            # Use LangChain enhanced search
            logger.info("Starting knowledge base search", query=query)
